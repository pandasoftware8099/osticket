...
$checks = "SELECT * FROM osticket.ost_ticket WHERE status_id = '2' AND staff_id = '".$thisstaff->getId()."'";
$check1 = db_query($checks);
$check_row = $check1->num_rows;
...
if($_REQUEST['id'] || $_REQUEST['number']) {
    ...
    elseif($_REQUEST['id'] && $check_row > 0) {
         if($ticket->getOwnerId() != $thisstaff->getstaffuserId())
         {
            if($ticket->getStaffId() != $thisstaff->getId())
             {
                $errors['err']=sprintf(__('You have resolved ticket havent close.'));
                $ticket=null;
             }
         }
    }
    ...
    if($ticket && $ticket->getId()) {
        ...
        case 'reply':
            ...
            else
            {
                ...
                if(!$errors && ($response=$ticket->postReply($vars, $type, $errors, $_POST['emailreply']))) {
                    if($ticket->getStatusId() == 2)
                    {
                        if($_POST['solve'] == '1')
                        {
                            $close = "UPDATE ost_ticket set status_id = '3', closed = NOW() WHERE ticket_id = '".$ticket->getId()."'";
                            $close1 = db_query($close);
                        }
                        else if($_POST['solve'] == '0')
                        {
                            $close = "UPDATE ost_ticket set status_id = '6' WHERE ticket_id = '".$ticket->getId()."'";
                            $close1 = db_query($close);
                        }
                    }
                ...
                /*if(!$errors)
                {
                    $sql = "INSERT INTO osticket.ost_thread_entry ( thread_id, user_id, type, flags, poster, body, ip_address, created, updated ) VALUES ( (SELECT id FROM osticket.ost_thread WHERE object_id = '".$ticket->getId()."'), '".$thisstaff->getstaffuserId()."', 'M', '64', (SELECT name FROM osticket.ost_user WHERE id = '".$thisstaff->getstaffuserId()."'), '".$vars['response']."', '".$_SERVER['REMOTE_ADDR']."', NOW(), NOW() )";
                    $query = db_query($sql);

                    if($ticket->getStatusId() == '2')
                    {
                        $sql2 = "UPDATE osticket.ost_ticket SET status_id = '3' WHERE ticket_id = '".$ticket->getId()."'";
                        $query2 = db_query($sql2);
                    }

                    if($ticket->getStaffId() != '0' && $vars['emailreply'] == '1')
                    {
                        $sql3 = "SELECT b.firstname, b.lastname, a.number, b.email AS staff_email, c.name AS client_name, d.name AS department_name FROM osticket.ost_ticket AS a INNER JOIN osticket.ost_staff AS b ON a.staff_id = b.staff_id INNER JOIN osticket.ost_user AS c ON b.staff_user_id = c.id INNER JOIN osticket.ost_department AS d ON a.dept_id = d.id WHERE a.ticket_id = '".$ticket->getId()."'";
                        $query3 = db_query($sql3);

                        foreach ($query3 as $result3) {
                            Email::send($result3['staff_email'], "New Message Alert", " <h3><strong>Hi ".$result3['firstname']." ".$result3['lastname'].",</strong></h3> New message appended to ticket <a href='http://panda-estore.com/helpdesk/scp/tickets.php?id=".$ticket->getId()."'>#".$result3['number']."</a> <br /><br /><table><tbody> <tr> <td> <strong>From</strong>: </td> <td> ".$result3['client_name']." </td> </tr> <tr> <td> <strong>Department</strong>: </td> <td> ".$result3['department_name']." </td> </tr> <tr> <td><strong>Help Topic:</strong></td> <td>".$ticket->getHelpTopic()." </td> </tr> <tr> <td><strong>Sub Topic:</strong></td> <td>".$ticket->getSubject()."</td> </tr> </tbody></table> <br /> ".$vars['response']." <br /><br /><hr /> To view or respond to the ticket, please <a href='http://panda-estore.com/helpdesk/scp/tickets.php?id=".$ticket->getId()."'><span style='color:rgb(84, 141, 212)'>login</span></a> to the support ticket system", '', '');
                        }
...
if($ticket) {
    if($check_row == 0)
    {
        if($thisstaff->getDeptId() == $ticket->getDeptId() && $ticket->getOwnerId() != $thisstaff->getstaffuserId())
        {
            $sql = "UPDATE osticket.ost_ticket set status_id = '6', staff_id = '".$thisstaff->getId()."' WHERE number = '".$ticket->getNumber()."' AND status_id = '7' OR status_id = '8' OR status_id = '9'";
            $query = db_query($sql);
        }
    }
