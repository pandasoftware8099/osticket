...
// Tabs
$(document).on('click.tab', 'div > ul.tabs > li > a', function(e) {
    e.preventDefault();
    var $this = $(this),
        $ul = $(this).closest('div'),
...
$(document).on('click.tab_1', 'div > ul.tabs_1 > li > a', function(e) {
    e.preventDefault();
    var $this = $(this),
        $ul = $(this).closest('div'),
        $container = $('#'+$ul.attr('id')+'_container');
    ...
    if (!$tab.length && $(this).data('url').length > 1) {
        ...
        $tab.addClass('tab_content_1');
        // Don't change the URL hash for nested tabs or in dialogs
        if ($(this).closest('.tab_content_1, .dialog').length == 0)
            $.changeHash($(this).attr('href'), true);
    }

    if ($tab.length) {
        $('ul.tabs_1').children().removeClass('active');
        $(this).closest('li').addClass('active');
        $('.tab_content_1').hide();
        $tab.fadeIn('fast');
        return false;
...
$(document).on('click.tab_2', 'div > ul.tabs_2 > li > a', function(e) {
    e.preventDefault();
    var $this = $(this),
        $ul = $(this).closest('div'),
        $container = $('#'+$ul.attr('id')+'_container');
    ...
    if (!$tab.length && $(this).data('url').length > 1) {
        ...
        $tab.addClass('tab_content_2');
        // Don't change the URL hash for nested tabs or in dialogs
        if ($(this).closest('.tab_content_2, .dialog').length == 0)
            $.changeHash($(this).attr('href'), true);
    }

    if ($tab.length) {
        $('ul.tabs_2').children().removeClass('active');
        $(this).closest('li').addClass('active');
        $('.tab_content_2').hide();
        $tab.fadeIn('fast');
        return false;
